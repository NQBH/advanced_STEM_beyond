...
def find_empty_mrv(board):
    """
    Finds an empty cell in the sudoku board using the Minimum Remaining Values (MRV) heuristic.
    It returns the empty cell with the fewest valid options.

    Args:
        board (list[list[int]]): A 9x9 sudoku board represented as a list of lists of integers.

    Returns:
        tuple[int, int]|None: The position of the empty cell with the minimum remaining values as a tuple, or None if no empty cell is found.
    """
    min_options = 10  # A number larger than any possible options (1-9)
    best_pos = None

    for r in range(9):
        for c in range(9):
            if board[r][c] == 0:
                # Count valid options for this empty cell
                current_options = 0
                for num_to_try in range(1, 10):
                    if valid(board, (r, c), num_to_try):
                        current_options += 1

                # If we found an empty cell with 0 options, it's an immediate dead end
                if current_options == 0:
                    return (r, c) # Return immediately as this path is invalid

                if current_options < min_options:
                    min_options = current_options
                    best_pos = (r, c)
    return best_pos
