...
\begin{abstract}
    ...
    Latest version:
    \begin{itemize}
        \item {\it Matrix Multiplication \& Fast Doubling Techniques in Competitive Programming}.

        PDF: {\sc url}: \url{https://github.com/NQBH/advanced_STEM_beyond/blob/main/OLP_ICPC/matrix_multiplication/NQBH_matrix_multiplication.pdf}.

        \TeX: {\sc url}: \url{https://github.com/NQBH/advanced_STEM_beyond/blob/main/OLP_ICPC/matrix_multiplication/NQBH_matrix_multiplication.tex}.
    \end{itemize}
\end{abstract}
\tableofcontents
...
\begin{problem}[Baltic OI 2007, G\"ustrow, Germany, connected point]
    Consider a regular grid of $3\times n$ points. Every point in the grid has up to $8$ neighboring points. We are interested in counting the number of different ways to connect the points of the grid to form a polygon that fulfills the following conditions:
    \begin{enumerate}
        \item The set of vertices of the polygon consists of all $3\times n$ points.
        \item Adjacent vertices of the polygon are neighboring points in the grid.
        \item Each polygon is simple, i.e. there must not be any self-intersections.
    \end{enumerate}
    Write a program that calculates for a given $n$ the number of possible ways to connect the points as described modulo $10^9$.
    \item {\sf Input.} The input is read from a text file named {\tt points.in}. The 1st \& only line contains 1 positive integer $n\in[10^9]$.
    \item {\sf Output.} The output is written into a text file named {\tt points.out}. The only line to be written contains the remainder of the number of ways to connect the points modulo $10^9$.
    \item {\sf Grading.} 30\% of the test cases have values of $n\le200$. 70\% of the test cases have values of $n\le10^5$.
\end{problem}
