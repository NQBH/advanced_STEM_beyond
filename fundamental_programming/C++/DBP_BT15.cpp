...
//BT2
void BT2(long long d, long long r) {
    while (d <= 0 || r <= 0) {
        cout << "loi so, vui long nhap lai " << endl;
        cin >> d >> r;
    }
...
//BT4
void BT4 (long long x, long long y) {
    //sqrt
    cout << "sqrt: ";
...
//BT5
void BT5 (long long a, long long b) {
    cout << "ket qua: " << endl;
    if (a == 0)  {
...
//BT6
void BT6(long long a, long long b, long long c) {
    if (a == 0) {
        //bx + c = 0
        if (b == 0) {
            if (c == 0) {
...
//BT7
void BT7(long long a, long long b, long long c) {
    double xmin;
    double xmax;
...
//BT8
void BT8(int duong, int am, int ko, int n, double num) {
    for (int i = 0; i < n; i++) {
        cout << "nhap so: ";
        cin >> num;
...
//BT9
void BT9(long long n, int chan, int le) {
    vector<int> arr(n);
//nhap so
    for (int i = 0; i < n; i++) {
        cout << "nhap so: ";
...
//BT11
void BT11(long long a, long long b, long long c, long long A, long long B, long long C) {
    double D = (a * B) - (A * b);
    double Dx = (c * B) - (C * b);
    double Dy = (a * C) - (A * c);
    if (D != 0) {
...
void BT12(long long m, long long n, double kq = 0) {
    while (cin.fail() || n <= 0) {
        cin.clear();
        cin.ignore(1000, '\n');
...
void BT13(int n, int i, double m, double M) {
    double kq = 0;
    while ( i <= n) {
        if (kq + pow(i, m) <= M) {
            kq += pow(i, m);
...
void BT14(long long A, int a, int PIN, int max_trial, int dem, int inPIN) {
    dem = 0;
    while (cin.fail() || A < 0) {
        cin.clear();
        cin.ignore(1000, '\n');
...
int main() {
    ...
    cin >> O;
    BT14(O, o, max_trial, dem , PIN, inPIN);
    return 0;
}
