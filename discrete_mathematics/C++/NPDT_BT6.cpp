...
// (p -> r) and (q -> r) = (p or q) -> r
    for (int i = 0; i < 2; ++i) {
        ...
        bool left = (!p || r) and (!q || r);
        bool right = !(p || q) || r;

        if (left == right)
            ans = 1;
    }
    cout << "(p -> r) and (q -> r) = (p or q) -> r: " << ans << endl;
...
// (p -> r) or (q -> r) = (p and q) -> r
    ...
    cout << "(p -> r) or (q -> r) = (p and q) -> r: " << ans << endl << endl;
...
//p <-> q =  (p and q) or (!p and !q)
    ...
    cout << "p <-> q =  (p and q) or (!p and !q): " << ans << endl;
