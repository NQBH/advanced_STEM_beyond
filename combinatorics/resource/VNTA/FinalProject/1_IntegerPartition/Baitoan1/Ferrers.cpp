...
cur: index hiện tại -- current index
...
void genOrderedPartitions(int cur, int k, int nRemain, vector<int>& part, vector<vector<int>>& res) {
    ...
    int minVal = 1, maxVal = nRemain - (k - cur - 1);
...
void Ferrers(const vector<int>& part) { // print Ferrers diagram
    for (int val : part) {
...
void FerrersTranspose(const vector<int>& part) { // print Ferrers transpose diagram
    int maxHeight = *max_element(part.begin(), part.end()), k = part.size();
