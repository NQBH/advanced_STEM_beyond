...
elseif cases == 3
    ...
    N0=N;
    M=8;% number of iteration when refine mesh
    bc1=sin((5*pi)/16)^5*cos(1/2)^3;
    bc2=-sin(pi/16)^5*cos(3/2)^3;
    it=2;
end
...
integration=2; % Integration rule
if integration==1
...
for jj=1:M
    ...
    for i_iter=1:N
        ...
        if(i_iter==1)
            ...
            b(i_iter)=b(i_iter)-a1*bc1;
        else
            if(i_iter==N)
                A(i_iter,i_iter-1)=a1;
                A(i_iter,i_iter)=-(a1+b1);
                b(i_iter)=b(i_iter)-b1*bc2;
...
%     filename=sprintf('fig_dirichlet_result_G%d_CP%d_I%d_N%d_M%d_C%d.pdf',grid,cp,integration,N,M,cases);
...
if cases == 1
    plot(log(ll),-log(norml2),'r', log(ll), -log(norme),'blue', log(ll),1.5*log(ll)+2, 'black', log(ll), 2*log(ll)+3,'green', log(ll), log(ll)+2.5,'magenta');
elseif cases == 2
    plot(log(ll),-log(norml2),'r', log(ll), -log(norme),'blue', log(ll),1.5*log(ll)-6, 'black', log(ll), 2*log(ll)-4,'green', log(ll), log(ll)-5.5,'magenta');
elseif cases == 3
...
filename=sprintf('fig_dirichlet_error_G%d_CP%d_I%d_M%d_C%d.pdf',grid,cp,integration,M,cases);
orient(fig,'landscape');
fig.PaperPositionMode = 'auto';
saveas(fig,filename);
fprintf(filename); fprintf('\n');