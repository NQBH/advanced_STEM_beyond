...
\begin{enumerate}
	...
	\begin{itemize}
		\item $\ln x$: natural logarithm: $\log_ex$
		\item $\lg x$: binary logarithm: $\log_2x$
		\item $\log x$: common logarithm: $\log_{10}x$
		\item $x\mod y$: remainder: $x - y\lfloor\frac{x}{y}\rfloor$
		\item $\{x\}$: fractional part: $x\mod 1$
		\item $\sum f(x)\delta x$: indefinite summation
		\item $\sum_a^b f(x)\delta x$: definite summation
		\item $x^{\underline{n}}$: falling factorial power: $\frac{x!}{(x - n)!}$
		\item $x^{\overline{n}}$: rising factorial power: $\frac{\Gamma(x + n)}{\Gamma(x)}$
		\item subfactorial: $\frac{n!}{0!} - \frac{n!}{1!} + \cdots + (-1)^n\frac{n!}{n!}$
		\item $\Re z$: real par: $x$, if $z = x + iy$
		\item $\Im z$: imaginary part: $y$, if $z = x + iy$
		\item $H_n$: harmonic number $\sum_{i=1}^n \frac{1}{i}$
		\item $H_n^{(x)}$: generalized harmonic number $\sum_{i=1}^n \frac{1}{i^x}$
		\item $f^{(m)}(z)$: $m$th derivative of $f$ at $z$
		\item $\begin{bmatrix}
			n\\m
		\end{bmatrix}$: Stirling cycle number 1st kind
		\item $\begin{Bmatrix}
			n\\m
		\end{Bmatrix}$: Stirling subset number 2nd kind
		\item $\left\langle
		\begin{matrix}
			n\\m
		\end{matrix}\right\rangle$: Eulerian number
		\item $\left\langle\left\langle
		\begin{matrix}
			n\\m
		\end{matrix}\right\rangle\right\rangle$: 2nd-order Eulerian number
		\item $(a_m,\ldots,a_0)_b$: radix notation for $\sum_{k=0}^m a_kb^k$
		\item $K(a_1,\ldots,a_n)$: continuant polynomial
		\item $F\left(\begin{matrix}
			a,b\\c
		\end{matrix}|z\right)$: hypergeometric function
		\item $\#A$: cardinality: number of elements in set $A$
		\item $[z^n]f(z)$: coefficient of $z^n$ in $f(z)$
		\item $[\alpha..\beta]$: closed interval: set $\{x|\alpha\le x\le\beta\}$
		\item $[m = n]$: 1 if $m = n$, otherwise 0
		\begin{remark}
			In general, if $S$ is any statement that can be true or false, bracketed notation $[S]$ stands for $1$ if $S$ is true, $0$ otherwise.
		\end{remark}
		\item $[m\backslash n]$: 1 if $m$ divides $n$, otherwise 0
		\item $[m\backslash\backslash n]$: 1 if $m$ exactly divines $n$, otherwise 0
		\item $[m\bot n]$: 1 if $m$ is relatively prime to $n$, otherwise 0
	\end{itemize}
	Throughout this text, use single-quote marks (`$\ldots$') to delimit text as it is {\it written}, double-quote marks (``$\ldots$'') for a phrase as it is {\it spoken}. Thus, string of letters `string' is sometimes called a ``string.'' An expression of form `a/bc' means same as `a/(bc)'. Moreover, $\log x/\log y = (\log x)/(\log y)$ \& $2n! = 2(n!)$.
	...
	\begin{itemize}
		\item {\sf1. Recurrent Problems.} Explore 3 sample problems that give a feel for what's to come. They have 2 traits in common: They've all been investigated repeatedly by mathematicians; \& their solutions all use idea of {\it recurrence}, in which solution to each problem depends on solutions to smaller instances of same problem.
		\begin{itemize}
			\item {\sf1.1. Tower of Hanoi.}
			\item {\sf1.2. Lines in Plane.}
			\item {\sf1.3. Josephus Problem.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf2. Sums.}
		\begin{itemize}
			\item {\sf2.1. Notations.}
			\item {\sf2.2. Sums \& Recurrences.}
			\item {\sf2.3. Manipulation of Sums.}
			\item {\sf2.4. Multiple Sums.}
			\item {\sf2.5. General Methods.}
			\item {\sf2.6. Finite \& Infinite Calculus.}
			\item {\sf2.7. Infinite Sums.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf3. Integer Functions.}
		\begin{itemize}
			\item {\sf3.1. Floors \& Ceilings.}
			\item {\sf3.2. Floor{\tt/}Ceiling Applications.}
			\item {\sf3.3. Floor{\tt/}Ceiling Recurrences.}
			\item {\sf3.4. `mod': Binary Operation.}
			\item {\sf3.5. Floor{\tt/}Ceiling Sums.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf4. Number Theory.}
		\begin{itemize}
			\item {\sf4.1. Divisibility.}
			\item {\sf4.2. Primes.}
			\item {\sf4.3. Prime Examples.}
			\item {\sf4.4. Factorial Factors.}
			\item {\sf4.5. Relative Primality.}
			\item {\sf4.6. `mod': Congruence Relation.}
			\item {\sf4.7. Independent Residues.}
			\item {\sf4.8. Additional Applications.}
			\item {\sf4.9. Phi \& Mu.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf5. Binomial Coefficients.}
		\begin{itemize}
			\item {\sf5.1. Basic Identities.}
			\item {\sf5.2. Basic Practices.}
			\item {\sf5.3. Tricks of Trade.}
			\item {\sf5.4. Generating Functions.}
			\item {\sf5.5 Hypergeometric Functions.}
			\item {\sf5.6. Hypergeometric Transformations.}
			\item {\sf5.7. Partial Hypergeometric Sums.}
			\item {\sf5.8. Mechanical Summation.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf6. Special Numbers.}
		\begin{itemize}
			\item {\sf6.1. Stirling Numbers.}
			\item {\sf6.2. Eulerian Numbers.}
			\item {\sf6.3. Harmonic Numbers.}
			\item {\sf6.4. Harmonic Summation.}
			\item {\sf6.5. Bernoulli Numbers.}
			\item {\sf6.6. Fibonacci Numbers.}
			\item {\sf6.7. Continuants.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf7. Generating Functions.}
		\begin{itemize}
			\item {\sf7.1. Domino Theory \& Change.}
			\item {\sf7.2. Basic Maneuvers.}
			\item {\sf7.3. Solving Recurrences.}
			\item {\sf7.4. Special Generating Functions.}
			\item {\sf7.5. Convolutions.}
			\item {\sf7.6. Exponential Generating Functions.}
			\item {\sf7.7. Dirichlet Generating Functions.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf8. Discrete Probability.}
		\begin{itemize}
			\item {\sf8.1. Defs.}
			\item {\sf8.2. Mean \& Variance.}
			\item {\sf8.3. Probability Generating Functions.}
			\item {\sf8.4. Flipping Coins.}
			\item {\sf8.5. Hashing.}
			\item {\sf Exercises.}
		\end{itemize}
		\item {\sf9. Asymptotics.}
		\begin{itemize}
			\item {\sf9.1. A Hierarchy.}
			\item {\sf9.2. O Notation.}
			\item {\sf9.3. O Manipulation.}
			\item {\sf9.4. 2 Asymptotic Tricks.}
			\item {\sf9.5. Euler's Summation Formula.}
			\item {\sf9.6. Final Summations.}
			\item {\sf Exercises.}
		\end{itemize}
	\end{itemize}	
