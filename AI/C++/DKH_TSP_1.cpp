...
int main() {
    ...
    // Điểm bắt đầu luôn là thành phố thứ nhất (chỉ số 0)
    int diemBatDau = 0; // Thay đổi ở đây

    // Không cần vòng lặp for cho diemBatDau nữa, chỉ thực hiện một lần
    ...
    // Cập nhật chu trình ngắn nhất nếu tìm thấy (trong trường hợp này, chỉ có một chu trình)
    ...
    // In kết quả
    cout << "Chu trinh ngan nhat theo heuristic Nearest Neighbor (bat dau tu diem 0): ";
    for (int i = 0; i < chuTrinhNganNhat.size(); ++i) {
        cout << chuTrinhNganNhat[i] << (i == chuTrinhNganNhat.size() - 1 ? "" : " -> ");
    }
    cout << endl;
    cout << "Do dai chu trinh: " << doDaiChuTrinhNganNhat << endl;
