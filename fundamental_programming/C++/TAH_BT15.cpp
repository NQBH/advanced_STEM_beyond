...
void bt2 (double a , double b) {
    cout << "Hinh chu nhat" << endl;
    cout << "CV " << (a + b) * 2 << " " << "DT " << a * b << endl ;
}
void bt3 (double a, double b, double c) {
    if ( a == b && b == c) {
        cout << "HLP" << endl;
    }
    else {
...
void bt4(double x, double y, double A, double B, double C) {
    // sqrt
    if (x >= 0) cout << sqrt(x) << "\n";
    else cout << "Error\n";
    // pow
...
int main() {
    double a, b, c, x, y, A, B, C; int n, m, M;
    cin >> a >>  b >> c >> x >> y >> A >> B >> C >> n >> m >> M;
    bt1 (a);
    bt2 (a, b);
    bt3 (a, b, c);
    bt4 (x, y, A, B, C);
