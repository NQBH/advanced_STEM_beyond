...
optimal_schedule = cp_sat_optimizer(instance)
            ...
            \begin{itemize}
                ...
                \item Ngẫu nhiên: Chọn một thao tác ngẫu nhiên từ các thao tác khả dụng.
            \end{itemize}
            ...
            -- 1 lưu ý quan trọng: bằng cách xác định thời điểm một thao tác được chọn là ``khi máy trở nên nhàn rỗi'', ngầm loại trừ khả năng máy phải chờ một thao tác có khả năng hiệu quả hơn nhưng chưa sẵn sàng. Do đó, định nghĩa này cũng hoạt động như một phương pháp tìm kiếm giới hạn không gian hành động khả dụng thành các thao tác có thể bắt đầu ngay lập tức.
...
schedule_mwkr = solver_mwkr(instance)
    ...
    \begin{itemize}
        ...
        -- Việc huấn luyện một mạng nơ-ron nhân tạo bao gồm việc tối ưu hóa các tham số của nó (trọng số $W$ \& độ lệch ${\bf b}$) để giảm thiểu hàm mất mát $L$, hàm này đo lường sự khác biệt giữa dự đoán của mạng \& giá trị thực tế. Việc tối ưu hóa này được thực hiện bằng thuật toán lan truyền ngược (Rumelhart \& cộng sự, 1986), cung cấp một cách hiệu quả để tính toán độ dốc của hàm mất mát theo các tham số mạng bằng cách áp dụng quy tắc chuỗi một cách đệ quy. Các độ dốc này sau đó được sử dụng bởi một thuật toán tối ưu hóa, ví dụ như Stochastic Gradient Descent (SGD) hoặc các biến thể của nó như Adam (Kingma \& Ba, 2015), để cập nhật các tham số theo hướng giảm thiểu mất mát.
        ...
        \begin{itemize}
            ...
            -- Tóm lại, một lớp GAT với sự chú ý đa đầu hoạt động như sau: mỗi nút lân cận $j\in{\cal N}_i$ gửi một vectơ hệ số chú ý $\boldsymbol{\alpha}_{ij} = [\alpha_{ij}^{(1)},\ldots,\alpha_{ij}^{(K)}]$, trong đó mỗi phần tử tương ứng với một đầu chú ý khác nhau. Các hệ số này được sử dụng để tính $K$ tổ hợp tuyến tính độc lập của các đặc trưng ${\bf h}_j$ của các nút lân cận. Sau đó, các biểu diễn này được tổng hợp (thường thông qua phép nối hoặc lấy trung bình) để thu được biểu diễn cập nhật của nút $i$, được ký hiệu là ${\bf h}_i^{(l + 1)}$.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{itemize}
            -- SMDP được định nghĩa tương tự như MDP nhưng bao gồm một thành phần bổ sung cho thời lượng của các hành động. Về mặt hình thức, SMDP là một bộ $({\cal S},{\cal A},P,R,F,\gamma)$ trong đó
            \begin{itemize}
                \item ${\cal S},{\cal A},P,R,\gamma$ được định nghĩa như trong MDP.
                \item $F:{\cal S}\times{\cal A}\times{\cal A}\times\mathbb{R}^+\to[0,1]$ là phân phối thời gian lưu trú, đo thời gian ở trong một trạng thái trước khi chuyển sang trạng thái khác. Tức là, $F(s,a,s',\tau)$ biểu diễn xác suất trạng thái chuyển đổi từ $s$ sang $s'$, dưới tác động $a$, mất thời gian $\tau$.
            \end{itemize}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}
                ...
                \end{equation*}
                là tỷ lệ xác suất giữa chính sách mới \& cũ, $B$: kích thước lô, \& $\epsilon$ là một siêu tham số giới hạn mức độ thay đổi chính sách, thường là 0,2.
            \end{enumerate}
        \end{itemize}
    ...
    \begin{itemize}
        ...
        -- Các công trình khác cũng đã phát triển môi trường RL để hỗ trợ các thí nghiệm của họ (Zhang và cộng sự, 2020; Song và cộng sự, 2023). Tuy nhiên, chúng cũng chỉ hỗ trợ các lựa chọn thiết kế của riêng chúng (ví dụ: một tập hợp các đặc điểm nút hoặc biểu diễn đồ thị cụ thể). Tương tự, chúng thiếu tính mô-đun \& tài liệu hướng dẫn để tùy chỉnh môi trường hơn nữa. Các tác giả khác chưa công khai mã của họ.
