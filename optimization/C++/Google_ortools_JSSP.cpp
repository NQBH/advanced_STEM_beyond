...
int64_t horizon = 0;
for (const auto& job : jobs_data)
	for (const auto& [_, time] : job) horizon += time;
...
for (int job_id = 0; job_id < jobs_data.size(); ++job_id) {
	...
	for (int task_id = 0; task_id < job.size(); ++task_id) {
		...
		IntVar start = cp_model.NewIntVar({0, horizon}).WithName(std::string("start") + suffix);
		IntVar end = cp_model.NewIntVar({0, horizon}).WithName(std::string("end") + suffix);
		IntervalVar interval = cp_model.NewIntervalVar(start, duration, end).WithName(std::string("interval") + suffix);
...
// define constraints
// create & add disjunctive constraints
for (const auto machine : all_machines)
	cp_model.AddNoOverlap(machine_to_intervals[machines]);
...
if (response.status() == CpSolverStatus::OPTIMAL || response.status() == CpSolverStatus::FEASIBLE) {
	LOG(INFO) << "Solution:";
	// create 1 list of assigned tasks per machine
	struct AssignedTaskType {
		...
		bool operator<(const AssignedTaskType& rhs) const {
			return std::tie(this->start, this->duration) < std::tie(rhs.start, rhs.duration);
		}
	};
	...
	for (const auto machine : all_machines) {
		...
		for (const auto& assigned_task : assigned_jobs[machine]) {
			std::string name = absl::StrFormat("job_%d_task_%d", assigned_task.job_id, assigned_task.task_id);
			// add spaces to output to align columns
			sol_line_tasks += absl::StrFormat("%-15s", name);

			int64_t start = assigned_task.start;
			int64_t duration = assigned_task.duration;
			std::string sol_tmp = absl::StrFormat("[%i,%i]", start, start + duration);
			// add spaces to output to align columns
