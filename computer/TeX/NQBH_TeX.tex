...
\begin{enumerate}
	\item \texttt{default units}: In the command \verb|\chemfig{A-[1]B-[7]C}| the parameters inside brackets set the angle in special units, each unit equals $45^\circ$. Hence in the example the angles are $45^\circ$ \& $315^\circ$.
	\item \texttt{absolute units}: The angles can be set in absolute units, in the command \verb|\chemfig{A-[:50]B-[:-25]C}| the parameter inside the brackets represent the angle, in degrees, measured from the horizontal baseline. Negative angles are allowed.
	\item \texttt{relative angles}: In the 3rd example \verb|\chemfig{A-[::50]B-[::-25]C}| the angles are measured from the previous bond, instead of the baseline.''
\end{enumerate}
...
\begin{enumerate}
	...
	\item \verb|\setatomstep{2em}|: The separation between atoms (nodes) in the formula is set to \texttt{2em}. Other \href{https://www.overleaf.com/learn/latex/Lengths_in_LaTeX}{\LaTeX\ units} can be used.
	\item \verb|\chemname{}{}|: The 1st parameter in this command is a \texttt{chemfig} formula, the 2nd one is some text that will be printed below the formula. In the example the text is ``Ethanal''.
\end{enumerate}
...
\begin{enumerate}
	\item \verb|\draw (-2,0) -- (2,0);|: This defines a line whose endpoints are \texttt{(-2,0)} \& \texttt{(2,0)}.
	\item \verb|\filldraw [gray] (0,0) circle (2pt);|: The point is created as a very small \texttt{gray circle} centered at \texttt{(0,0)} \& whose radius is \texttt{(2pt)}. The \verb|\filldraw| command is used to draw elements \& fill them with a specific color.
...
\end{center}
\begin{enumerate}
	\item \verb|\filldraw[color=red!60, fill=red!5, very thick](-1,0) circle (1.5);|: This command was used in the previous section to draw a \textit{point}, but in this instance there are some additional parameters inside the brackets. These are explained below:
\end{enumerate}
